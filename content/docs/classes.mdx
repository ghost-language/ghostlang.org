...
Classes define an objects _behavior_ and _state_. Behavior is defined by [methods](/docs/1.0/method-calls) which live in the class. Every object of the same class supports the same methods. State is defined in properties, whose values are stored in each instance.
...
All state stored in instances are stored in _properties_. Each property has a name, are bound to `this`, and act the same as variables.
...
In addition to class inheritance, Ghost supports _traits_. Traits are like classes, but they can't be instantiated. Instead, they're used to share methods and properties between classes.
